...
def build_benchmark_tab(self):
    ...
    scrollbar = ttk.Scrollbar(hist_inner, orient="vertical", command=self.history_tree.yview)
    self.history_tree.configure(yscrollcommand=scrollbar.set)
    self.history_tree.grid(row=0, column=0, sticky="nsew")
    scrollbar.grid(row=0, column=1, sticky="ns")
    hist_inner.columnconfigure(1, weight=0)
    self.history_tree.tag_configure("latest", foreground="#28a745", font=("Segoe UI", 9, "bold"))
    style = ttk.Style()
    ...
    self.start_btn = ttk.Button(right, text="▶ Start", bootstyle="success", command=self._start)
    self.start_btn.pack(side="left", padx=(0, 4))
    self.stop_btn = ttk.Button(right, text="⊠ Stop", bootstyle="danger", state="disabled", command=self._stop)
    self.stop_btn.pack(side="left", padx=(0, 4))
    ttk.Button(right, text="⇄ Reset", bootstyle="warning-outline", command=self._reset_log).pack(side="left", padx=(0, 4))
    ttk.Button(right, text="⊗ Close", bootstyle="danger-outline", command=self._close).pack(side="left")
