...
def open_core_picker(self):
    ...
    ttk.Label(header_frame, text="Thread Selection", font=("Segoe UI", 11, "bold")).pack(anchor="w")
    ttk.Label(header_frame, text="Toggle threads to include in the test run.", font=("Segoe UI", 9), foreground="#888888").pack(anchor="w", pady=(1, 0))
    ttk.Separator(win, orient="horizontal").pack(fill="x")
    # legend
    legend_frame = ttk.Frame(win, padding=(14, 6, 14, 0))
    legend_frame.pack(fill="x")
    ttk.Label(legend_frame, text="(P) = Physical  ·  (HT) = Hyper-Thread", font=("Segoe UI", 9), foreground="#888888").pack(anchor="w")
    # grid
    ...
    for grid_row, (core_id, threads) in enumerate(sorted(topology.items())):
        ...
        sep_frame.grid(row=grid_row * 2, column=0, columnspan=THREAD_COLS, sticky="ew", padx=4, pady=(10 if grid_row > 0 else 4, 3))
        ttk.Label(sep_frame, text=f"  CORE {core_id}", font=("Segoe UI", 10, "bold")).pack(side="left")
        ttk.Separator(sep_frame, orient="horizontal").pack(side="left", fill="x", expand=True, padx=(6, 0), pady=1)
        # thread buttons
        for col, thread_id in enumerate(sorted(threads)):
            ...
            btn = ttk.Button(grid_frame, text=label, width=11, bootstyle="success-outline" if is_enabled else "danger")
    ...
    footer.pack(fill="x")
    ttk.Button(footer, text="✔ Confirm", bootstyle="success-outline", width=12, command=lambda: [self.core_blacklist_var.set(",".join(str(t) for t in sorted(k for k, v in btn_vars.items() if not v.get()))), win.destroy()]).pack(side="right", padx=(4, 0))
    ttk.Button(footer, text="⊗ Close", bootstyle="danger-outline", width=12, command=win.destroy).pack(side="right")