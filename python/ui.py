...
from python.options import save_settings
...
def build_info_row(self, parent):
    ...
    self.os_label = ttk.Label(
        os_row, text=f" {platform.system()} {platform.release()}", style="InfoValue.TLabel"
    )
...
def build_controls(self, parent):
    ...
    configure_numeric_spinbox(
        self, ttk.Spinbox(runs_frame, from_=1, to=999, width=5, textvariable=self.loops_var)
    ).pack(side="left")
