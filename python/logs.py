...
from python.timers import reset_timer
...
def insert_log(widget, message, tag="info"):
    ...
    for match in re.finditer(ansi_pattern, full_message):
        ...
        last_pos = match.end()
    if remaining := full_message[last_pos:]:
...
def open_output_window(self):
    ...
    text_widget = scrolledtext.ScrolledText(
        container, wrap="word", font=("Segoe UI", 12)
    )
    ...
    ttk.Button(
        btn_frame, text="⊗ Close", bootstyle="danger-outline", command=window.destroy
    ).pack(side="right", padx=2)
    self.output_windows.append({"window": window, "text": text_widget})
        ...
        if lines:
            ...
            if test_name == "Starting...":
                self.clock_label_bottom.config(
                    text="▷ " + test_name + "\n" + progress, fg="#856404", bg="#fff3cd"
                )
            elif test_name == "Waiting...":
                self.clock_label_bottom.config(
                    text="⏚ " + test_name + "\n" + progress, fg="#343a40", bg="#e9ecef"
                )
            ...
            self.clock_label_bottom.config(text="No data", fg="#6c757d", bg="#f8f9fa")
    except Exception as e:
        self.clock_label_bottom.config(text="Error reading", fg="#721c24", bg="#f8d7da")
