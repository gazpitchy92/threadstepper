    ...
    if [[ ",$cpu_topology," == *"0"* || ",$cpu_topology," == *"1"* ]]; then
        ...
        for method in "${light[@]}"; do
            if [[ ",$core_blacklist," == *",$core,"* || ",$core_blacklist," == *",$core_second,"* ]]; then
                ...
                stress-ng --cpu 2 --taskset "$core,$core_second" --timeout "$light_time" --cpu-method "$method" > /dev/null 2>&1
                check_errors
                sleep "$rest_time"
            fi
        done
        for method in "${mixed[@]}"; do
            if [[ ",$core_blacklist," == *",$core,"* || ",$core_blacklist," == *",$core_second,"* ]]; then
                ...
                stress-ng --cpu 2 --taskset "$core,$core_second" --timeout "$medium_time" --cpu-method "$method" > /dev/null 2>&1
                check_errors
                sleep "$rest_time"
            fi
        done
        for method in "${heavy[@]}"; do
            if [[ ",$core_blacklist," == *",$core,"* || ",$core_blacklist," == *",$core_second,"* ]]; then
                ...
                stress-ng --cpu 2 --taskset "$core,$core_second" --timeout "$heavy_time" --cpu-method "$method" > /dev/null 2>&1
                check_errors
                sleep "$rest_time"
            fi
        done
        ...
        if [[ $core_next -lt $physical_cores ]]; then
            ...
            for method in "${light[@]}"; do
                if [[ ",$core_blacklist," == *",$core,"* || ",$core_blacklist," == *",$core_next,"* ]]; then
                    ...
                    stress-ng --cpu 2 --taskset "$core,$core_next" --timeout "$light_time" --cpu-method "$method" > /dev/null 2>&1
                    check_errors
                    sleep "$rest_time"
                fi
            done
            for method in "${mixed[@]}"; do
                if [[ ",$core_blacklist," == *",$core,"* || ",$core_blacklist," == *",$core_next,"* ]]; then
                    ...
                    stress-ng --cpu 2 --taskset "$core,$core_next" --timeout "$medium_time" --cpu-method "$method" > /dev/null 2>&1
                    check_errors
                    sleep "$rest_time"
                fi
            done
            for method in "${heavy[@]}"; do
                if [[ ",$core_blacklist," == *",$core,"* || ",$core_blacklist," == *",$core_next,"* ]]; then
                    ...
                    stress-ng --cpu 2 --taskset "$core,$core_next" --timeout "$heavy_time" --cpu-method "$method" > /dev/null 2>&1
                    check_errors
                    sleep "$rest_time"
                fi
            done
        ...
        if [[ $core_last -lt $(nproc) ]]; then
            ...
            for method in "${light[@]}"; do
                if [[ ",$core_blacklist," == *",$core_second,"* || ",$core_blacklist," == *",$core_last,"* ]]; then
                    ...
                    stress-ng --cpu 2 --taskset "$core_second,$core_last" --timeout "$light_time" --cpu-method "$method" > /dev/null 2>&1
                    check_errors
                    sleep "$rest_time"
                fi
            done
            for method in "${mixed[@]}"; do
                if [[ ",$core_blacklist," == *",$core_second,"* || ",$core_blacklist," == *",$core_last,"* ]]; then
                    ...
                    stress-ng --cpu 2 --taskset "$core_second,$core_last" --timeout "$medium_time" --cpu-method "$method" > /dev/null 2>&1
                    check_errors
                    sleep "$rest_time"
                fi
            done
            for method in "${heavy[@]}"; do
                if [[ ",$core_blacklist," == *",$core_second,"* || ",$core_blacklist," == *",$core_last,"* ]]; then
                    ...
                    stress-ng --cpu 2 --taskset "$core_second,$core_last" --timeout "$heavy_time" --cpu-method "$method" > /dev/null 2>&1
                    check_errors
                    sleep "$rest_time"
                fi
            done
