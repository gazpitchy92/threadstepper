...
browserTest() {
    ...
    echo "$(tput setaf 8)[DEBUG] Using electron: $(which electron)"

    for ((i = 0; i < browsers; i++)); do
        random_page=$((RANDOM % 7 + 1))
        ...
        if (( browsers > 1 )); then
            if (( i % 2 == 0 )); then
                ...
                taskset --cpu-list 0-$((half_cores - 1)) electron "$current_dir/tests/browser/launch.js" "$file_path" > /dev/null 2>&1 &
                browser_pids+=($!)
            else
                ...
                taskset --cpu-list $half_cores-$((num_cores - 1)) electron "$current_dir/tests/browser/launch.js" "$file_path" > /dev/null 2>&1 &
                browser_pids+=($!)
            fi
        else
            ...
            taskset --cpu-list 0-$((num_cores - 1)) electron "$current_dir/tests/browser/launch.js" "$file_path" > /dev/null 2>&1 &
            browser_pids+=($!)
        fi
        rest
    done
