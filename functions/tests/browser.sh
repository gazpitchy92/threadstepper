...
browserTest() {
    ...
    for ((i = 0; i < browsers; i++)); do
        ...
        if [[ "$first_half" == true || "$second_half" == true ]]; then
            ...
            if (( browsers > 1 )); then
                if (( (i + 1) % 2 == 0 )); then
                    echo "$(tput setaf 3)[DEBUG Browser $((i+1))] taskset --cpu-list 0-$((half_cores - 1)) ./tests/browser/$chromium_appimage $file_path$(tput sgr0)" | tee -a log.txt
                    taskset --cpu-list 0-$((half_cores - 1)) ./tests/browser/$chromium_appimage --new-window --no-sandbox --disable-gpu-sandbox --disable-dev-shm-usage --disable-gpu-compositing --disable-accelerated-2d-canvas --disable-accelerated-video-decode --disable-accelerated-video-encode --disable-webgl2 --num-raster-threads=1 --incognito --user-data-dir=$current_dir/tests/browser/tmp "$file_path" > /dev/null 2>&1 &
                else
                    echo "$(tput setaf 3)[DEBUG Browser $((i+1))] taskset --cpu-list $half_cores-$((num_cores - 1)) ./tests/browser/$chromium_appimage $file_path$(tput sgr0)" | tee -a log.txt
                    taskset --cpu-list $half_cores-$((num_cores - 1)) ./tests/browser/$chromium_appimage --new-window --no-sandbox --disable-gpu-sandbox --disable-dev-shm-usage --disable-gpu-compositing --disable-accelerated-2d-canvas --disable-accelerated-video-decode --disable-accelerated-video-encode --disable-webgl2 --num-raster-threads=1 --incognito --user-data-dir=$current_dir/tests/browser/tmp "$file_path" > /dev/null 2>&1 &
                fi
            else
                echo "$(tput setaf 3)[DEBUG] taskset --cpu-list 0-$((num_cores - 1)) ./tests/browser/$chromium_appimage $file_path$(tput sgr0)" | tee -a log.txt
                taskset --cpu-list 0-$((num_cores - 1)) ./tests/browser/$chromium_appimage --new-window --no-sandbox --disable-gpu-sandbox --disable-dev-shm-usage --disable-gpu-compositing --disable-accelerated-2d-canvas --disable-accelerated-video-decode --disable-accelerated-video-encode --disable-webgl2 --num-raster-threads=1 --incognito --user-data-dir=$current_dir/tests/browser/tmp "$file_path" > /dev/null 2>&1 &
            fi
            sleep $rest_time
        fi
    done
